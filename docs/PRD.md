...
- Begin with one global non-negative Fusion Policy. Select weights through a simplex grid search using cached route outputs, with target inclusion at the frozen rerank depth as the primary training signal and exact official TechnicalScore and post-rerank metrics as secondary evidence within the predeclared pool-recall tolerance.
